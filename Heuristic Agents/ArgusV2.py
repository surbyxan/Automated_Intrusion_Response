    ...
    def get_action(self, observation, action_space):
        ...
        for host_name in Offsets.PRIORITY_ORDER:
            ...
            if not observation[offset + 1]: # if not yet portscanned 
                if not self.DecoysDeployed[host_name][0]: # femitter not deployed
                    self.DecoysDeployed[host_name][0] = 1
                    return Offsets.ACTION_OFFSETS['decoy_femitter'] + Offsets.ACTION_HOST_OFFSET[host_name]
                elif not self.DecoysDeployed[host_name][1]: # haraka not deployed
                    self.DecoysDeployed[host_name][1] = 1
                    return Offsets.ACTION_OFFSETS['decoy_haraka'] + Offsets.ACTION_HOST_OFFSET[host_name]
